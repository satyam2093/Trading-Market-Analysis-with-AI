      ...
      <div className="flex flex-col sm:flex-row items-center gap-4">
        <div className="relative flex-1 w-full">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-3 rounded-xl bg-surface/90 border border-border/80 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-accent font-mono transition-colors shadow-sm"
          />
        </div>

        <div className="flex items-center rounded-xl border border-border/80 bg-surface/80 p-1 text-xs font-mono w-full sm:w-auto shadow-sm">
          {["ALL", "INDEX", "STOCK", "CRYPTO", "ETF"].map((t) => (
            <button
              key={t}
              onClick={() => setTypeFilter(t)}
              className={`px-4 py-2 rounded-lg transition-all flex-1 sm:flex-initial ${
                typeFilter === t
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {loading && assets.length === 0 ? (
          Array.from({ length: 9 }).map((_, i) => (
            <div key={i} className="p-6 rounded-2xl bg-surface/80 border border-border/80 space-y-3 animate-pulse">
          ...
          assets.map((asset) => (
            <Link
              ...
              className="p-5 rounded-2xl bg-surface/80 glass-card border border-border/80 hover:border-accent/40 transition-all flex items-center justify-between group shadow-sm hover:shadow-lg hover:-translate-y-0.5 duration-200"
            >
          ...
          <div className="col-span-full p-10 text-center bg-surface/80 rounded-2xl border border-border/80 glass-card space-y-4">
            ...
            {query.trim() && (
              <Link
                ...
                className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-foreground text-background text-xs font-mono font-medium hover:bg-foreground/90 transition-colors shadow-md"
              >
